...
function CoreNode({ id }: CoreNodeProps) {
  const [cashFlowVerbiage, currency, setCalculatorAmount] = useSpace(
    useShallow((state) => [
  ...
  const handleInflowClick = (e: React.MouseEvent) => {
    if (e.altKey) {
      e.preventDefault();
      e.stopPropagation();
      setCalculatorAmount(incomesTotal);
    }
  };

  const handleOutflowClick = (e: React.MouseEvent) => {
    if (e.altKey) {
      e.preventDefault();
      e.stopPropagation();
      setCalculatorAmount(expensesTotal);
    }
  };
  ...
  return (
    <div className='relative'>
      <div className='bg-surface-light dark:bg-surface-dark border-node-border relative z-10 flex h-48 w-40 items-center justify-end rounded-full border shadow-md'>
        ...
        <div 
          className='absolute top-0 left-0 flex h-1/2 w-full translate-y-1 flex-col items-center justify-center cursor-pointer'
          onClick={handleInflowClick}
          title='Alt+Click to add to calculator'
        >
        ...
        <div 
          className='absolute bottom-0 left-0 flex h-1/2 w-full -translate-y-1.5 flex-col items-center justify-center cursor-pointer'
          onClick={handleOutflowClick}
          title='Alt+Click to add to calculator'
        >
      ...
      <div 
        className='bg-surface-light dark:bg-surface-dark border-node-border absolute top-1/2 right-3 flex translate-x-full -translate-y-1/2 items-center justify-center rounded-r-full border p-4 shadow-md cursor-pointer'
